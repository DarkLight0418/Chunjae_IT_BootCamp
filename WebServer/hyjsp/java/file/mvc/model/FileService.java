...
public class FileService {
	...
	public void download(HttpServletRequest request, HttpServletResponse response, File f, String ofname) {
		try {
	        if (ofname == null || ofname.isEmpty()) {
	        	ofname = f.getName(); // fallback
	        }
			...
			if( Agent.indexOf("MSIE") >= 0 ){
				...
				if( IEV.equalsIgnoreCase("5.5") ){
					response.setHeader("Content-Disposition", "filename=" + new String( ofname.getBytes("utf-8"), "8859_1") );
				}else{
					response.setHeader("Content-Disposition", "attachment;filename="+new String(ofname.getBytes("utf-8"),"8859_1"));
				}
			}else{
				response.setHeader("Content-Disposition", "attachment;filename=" + new String( ofname.getBytes("utf-8"), "8859_1") );
			}
